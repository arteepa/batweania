...
### The Color Tokens
* **Primary (Earth & Bark):** `#5C4E38` (Primary) / `#2C2416` (Deepest Text)
* **Secondary (Forest Canopy):** `#4D6B3D` (Secondary) / `#9DC892` (Soft Leaf)
* **Tertiary (Amber Sun):** `#B87620` (Tertiary) / `#E4B458` (Warm Gold)
* **Surface (Bone & Parchment):** `#FAF4E8` (Background) / `#D4C496` (Brand Cream)
...
* **Base Layer:** `surface` (#FAF4E8)
* **Sub-sections:** `surface-container-low` (#F5EEE0) to create a subtle recessed feel.
* **Interactive Cards:** `surface-container-high` (#EAE3D2) to "lift" the element toward the user.

### Signature Textures & Soul
To elevate the UI beyond flat hex codes, apply a subtle, grainy noise overlay (opacity 3-5%) across all surfaces to mimic aged paper. Use "Atmospheric Gradients" for Hero sections: a radial transition from `primary-container` (#7D6D4E) to `primary` (#5C4E38) to simulate the depth of a canyon or a shaded grove.
...
* **Ambient Shadows:** If an element must float (e.g., a navigation bar or a moth-inspired fab), use a shadow color tinted with the `on-surface` color (#1E1A10) at 5% opacity with a very high blur (24px+). This mimics natural, diffused sunlight.
* **The "Ghost Border" Fallback:** If a boundary is required for accessibility, use the `outline-variant` (#CCC7B5) at 15% opacity. It should be felt, not seen.
...
* **Buttons:**
* **Primary:** Solid `primary` (#5C4E38) with `on-primary` (#ffffff) text. Keep corners square—no border radius.
* **Secondary:** No background; a `ghost border` (15% opacity) and `secondary` (#4D6B3D) text.
...
* **Inputs:** Use `surface-variant` for the field background. The active state should not be a bright blue, but a transition to a `tertiary` (#B87620) bottom-border (2px).
...
### Don’t:
* **Don’t use pure black (#000):** Use `on-background` (#1E1A10) for all "black" text to maintain the organic warmth.
